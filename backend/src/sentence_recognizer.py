...
class SentenceRecognizer:
    ...
    def predict(self, threshold: float | None = None, streak_threshold: int = 2, return_all: bool = False) -> tuple[str, float]:
        if not self._available or len(self._buffer) < max(10, int(self._seq_len * 0.34)):
            return "", 0.0
